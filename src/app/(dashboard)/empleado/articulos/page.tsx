  ...
  if (selected) {
    return (
      <div className="p-6 max-w-2xl mx-auto space-y-4">
        <button onClick={() => setSelected(null)} className="text-sm text-indigo-600 hover:underline">← Volver a artículos</button>
        <span className="text-xs bg-indigo-50 text-indigo-700 px-2 py-0.5 rounded-full capitalize">{selected.categoria}</span>
        <h1 className="text-2xl font-bold text-gray-900">{selected.titulo}</h1>
        <p className="text-xs text-gray-400">{selected.autor.nombre} {selected.autor.apellidos} · {format(new Date(selected.createdAt), "d 'de' MMMM yyyy", { locale: es })}</p>
        <div className="prose prose-sm max-w-none text-gray-700 leading-relaxed whitespace-pre-wrap">{selected.contenido}</div>
      </div>
    );
  }
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex gap-2 flex-wrap">
        {CATEGORIAS.map((c) => (
          <button key={c} onClick={() => setCat(c)} className={cn("px-3 py-1.5 rounded-full text-sm font-medium border transition-all capitalize", cat === c ? "bg-indigo-600 text-white border-indigo-600" : "bg-white text-gray-600 border-gray-200 hover:border-indigo-300")}>{c}</button>
        ))}
      </div>
      {loading ? (
          ...
          {filtrados.map((a) => (
            <Card key={a.id} className="hover:shadow-md transition-all cursor-pointer" onClick={() => setSelected(a)}>
              <CardContent className="pt-4 pb-4">
                <span className="text-xs bg-indigo-50 text-indigo-700 px-2 py-0.5 rounded-full capitalize">{a.categoria}</span>
