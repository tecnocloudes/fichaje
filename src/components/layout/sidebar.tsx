...
export function Sidebar({
      ...
      return (
        <Link
          ...
          className={cn(
            ...
            "text-indigo-300/50 hover:text-indigo-300/70",
            collapsed && "justify-center px-2"
          )}
          title={collapsed ? item.label : undefined}
        >
          <Icon className="h-4 w-4 shrink-0 text-indigo-500/40" />
          {!collapsed && (
            <>
  ...
  return (
    <>
      ...
      <aside
        className={cn(
          "fixed inset-y-0 left-0 z-30 flex flex-col bg-[#1e1b4b] text-[#e0e7ff] transition-all duration-300 overflow-hidden",
      ...
      >
        {/* Header */}
        <div className="flex h-14 items-center justify-between px-3 border-b border-[#312e81] shrink-0">
          {!collapsed && (
            <div className="flex items-center gap-2 overflow-hidden">
              <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-indigo-500">
                <Building2 className="h-4 w-4 text-white" />
              </div>
              <span className="font-bold text-sm text-white truncate">HR Suite</span>
            </div>
          )}
          {collapsed && (
            <div className="mx-auto flex h-7 w-7 items-center justify-center rounded-lg bg-indigo-500">
              <Building2 className="h-4 w-4 text-white" />
            </div>
          )}
          <button
        ...
        {/* User info */}
        {!collapsed ? (
          <div className="px-3 py-3 border-b border-[#312e81] shrink-0">
          ...
          <div className="flex justify-center py-3 border-b border-[#312e81] shrink-0">
        ...
        {/* Nav */}
        <nav className="flex-1 overflow-y-auto py-2 space-y-0.5 px-2 scrollbar-thin scrollbar-thumb-indigo-800">
              ...
              <div key={section.key} className="mb-1">
                {!collapsed && (
                  <button
                    ...
                    className="w-full flex items-center justify-between px-3 py-1.5 text-[10px] font-bold text-indigo-400/70 uppercase tracking-wider hover:text-indigo-300 transition-colors"
                  >
        ...
        {/* Bottom */}
        <div className="mt-auto border-t border-[#312e81] p-2 space-y-0.5 shrink-0">
          <button
            className={cn(
              "group flex w-full items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium text-indigo-200 hover:bg-[#312e81] hover:text-white transition-colors",
              collapsed && "justify-center px-2"
            )}
            title={collapsed ? "Notificaciones" : undefined}
          >
            <div className="relative shrink-0">
              <Bell className="h-4 w-4 text-indigo-400 group-hover:text-white transition-colors" />
              {notificationCount > 0 && (
          ...
          {user.rol === "SUPERADMIN" && (
            <Link
              ...
              className={cn(
                "group flex w-full items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium text-indigo-200 hover:bg-[#312e81] hover:text-white transition-colors",
                isActive("/admin/configuracion") && "bg-indigo-600 text-white",
                collapsed && "justify-center px-2"
              )}
          ...
          <button
            ...
            className={cn(
              "group flex w-full items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium text-indigo-200 hover:bg-red-500/20 hover:text-red-300 transition-colors",
              collapsed && "justify-center px-2"
            )}
            title={collapsed ? "Cerrar sesión" : undefined}
          >
            <LogOut className="h-4 w-4 shrink-0 text-indigo-400 group-hover:text-red-400 transition-colors" />
